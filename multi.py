...
data = pd.read_csv(
    '/Users/liri0/Documents/Studium/Semester5/SP/preprocessed_results_ems.txt',
    sep=',', header=None)
data.columns = ['Tweet'] + [f'Label{i + 1}' for i in range(9)]  # Spalten benennen

additional_data = pd.read_csv(
    '/Users/liri0/Documents/Studium/Semester5/SP/preprocessed_results_GoldStandardGerman.txt',
    sep=',', header=None)
...
for fold, (train_index, test_index) in enumerate(kf.split(X)):
    ...
    # Naive Bayes Modell für jedes Label
    ...
    for label_idx, label in enumerate(labels.columns):
        precision_macro = precision_score(y_test.iloc[:, label_idx], y_pred_combined[:, label_idx], average='macro',
                                          zero_division=0)
        recall_macro = recall_score(y_test.iloc[:, label_idx], y_pred_combined[:, label_idx], average='macro',
                                    zero_division=0)
    ...
    print(
        f"  Fold {fold + 1}: Macro Precision = {combined_precision:.4f}, Macro Recall = {combined_recall:.4f}, Macro F1 = {combined_f1_macro:.4f}, Micro F1 = {combined_f1_micro:.4f}, Weighted F1 = {combined_f1_weighted:.4f}")
"""
