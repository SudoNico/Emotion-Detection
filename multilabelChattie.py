...
for fold_idx, (train_index, test_index) in enumerate(kf.split(X_combined)):
    try:
        print(f"Fold {fold_idx+1}...")
        
        # Training und Test-Daten für den aktuellen Fold
        X_train, X_test = X_combined[train_index], X_combined[test_index]
        y_train, y_test = Y[train_index], Y[test_index]
        
        # Sicherstellen, dass die Labels Integer sind
        y_train = y_train.astype(int)
        y_test = y_test.astype(int)
        
        # Zielanzahl von Samples für jedes Label
        target_samples = 2738  # Beispielwert, kann angepasst werden
        
        # Resampling mit strict_ml_smote
        X_resampled, y_resampled = strict_ml_smote(X_train, y_train, target_samples=target_samples, k_neighbors=5)
        
        # Validierung der Resampled-Daten
        if len(X_resampled) != len(y_resampled):
            raise ValueError("X_resampled und y_resampled haben nicht die gleiche Länge!")
        ...
        print(f"Fold {fold_idx+1} abgeschlossen - Precision: {precision:.4f}, Recall: {recall:.4f}, F1-Score: {f1:.4f}")
    
    except Exception as e:
        print(f"Fehler im Fold {fold_idx+1}: {e}")
        continue  # Zum nächsten Fold springen
